...
class PesananController extends Controller
{
    public function index(Request $request)
    {
        ...
        try {
            ...
            // 🔹 Jika ada multitenant_id, ambil semua transaksi yang punya multitenant_id tersebut (kecuali refund_selesai)
            if ($multiIds->isNotEmpty()) {
                $extraTransaksi = Transaksi::with(['listTransaksiDetail.menus.tenants', 'user'])
                    ...
                    ->where('status', '!=', 'refund_selesai') // ⛔ Filter di sini juga
                    ->get();
            ...
            if ($status === 'diantar') {
                ...
                if ($transaksi->isPriority == 0) {
                    ...
                    if ($transaksi->multitenant_id) {
                        ...
                        $allReadyToDeliver = $relatedTransaksis
